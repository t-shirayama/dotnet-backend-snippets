...
- 成功・失敗を型で表したい → `Result<T>` / `Success<T>` / `Failure<T>`
- 値がない可能性を表したい → `Maybe<T>`
- null を検証してから非 null として扱いたい → `RequireNonNull`
- enum を安全に parse したい → `TryParseOrderStatus`
- 状態ごとの説明を pattern matching で返したい → `DescribeStatus`
