...
18. `IAsyncEnumerable<T>` をページングしたい → 追加候補
19. `Task.WhenAll` で複数処理を安全に待ちたい → 追加候補
20. キャンセル可能な非同期メソッドを書きたい → 追加候補
...
23. `IOptions<T>` を登録したい → 追加候補
24. `IOptionsSnapshot<T>` と `IOptionsMonitor<T>` を使い分けたい → 追加候補
25. 環境変数で設定を上書きしたい → 追加候補
...
29. HostedService を DI で動かしたい → 追加候補
...
33. 例外付きログを出したい → 追加候補
34. `BeginScope` でリクエスト単位の情報を持たせたい → 追加候補
35. correlation id をログに入れたい → 追加候補
36. health check を追加したい → 追加候補
37. OpenTelemetry の trace を追加したい → 追加候補
38. メトリクスをカウンターで出したい → 追加候補
39. slow query や遅い処理をログに出したい → 追加候補
40. 本番ではログレベルを設定で変えたい → 追加候補

## HttpClient・外部 API

41. `HttpClient` を DI で登録したい → 追加候補
42. `IHttpClientFactory` で typed client を作りたい → 追加候補
...
47. bearer token を付けて呼びたい → 追加候補
48. query string を安全に組み立てたい → 追加候補
49. `EnsureSuccessStatusCode` の失敗を扱いたい → 追加候補
50. API レスポンスの null や不正 JSON を検証したい → 追加候補

## ASP.NET Core API

51. Minimal API の endpoint を定義したい → 追加候補
52. Controller で `ActionResult<T>` を返したい → 追加候補
53. `ProblemDetails` でエラー応答を統一したい → 追加候補
54. global exception handler を追加したい → 追加候補
55. request body を DTO として受けたい → 追加候補
56. route parameter と query parameter を受けたい → 追加候補
57. endpoint filter を使いたい → 追加候補
58. middleware を作りたい → 追加候補
59. CORS を設定したい → 追加候補
...
69. validation error を `ProblemDetails` に変換したい → 追加候補
70. domain exception を HTTP status に変換したい → 追加候補

## EF Core・データアクセス

71. DbContext を DI に登録したい → 追加候補
72. SQLite in-memory で repository をテストしたい → 追加候補
73. InMemory provider と SQLite in-memory を使い分けたい → 追加候補
74. `AsNoTracking` を使いたい → 追加候補
75. pagination query を書きたい → 追加候補
76. `Include` と projection を使い分けたい → 追加候補
77. optimistic concurrency を扱いたい → 追加候補
78. transaction を使いたい → 追加候補
79. migration を追加・適用したい → 追加候補
80. soft delete の query filter を書きたい → 追加候補
...
85. password hash を検証したい → 追加候補
86. API key authentication を実装したい → 追加候補
87. rate limiting を設定したい → 追加候補
88. secret を設定ファイルに直書きしないようにしたい → 追加候補
89. input を HTML/SQL 用途に応じて扱いたい → 追加候補
90. file upload の拡張子・サイズ・MIME を検証したい → 追加候補
