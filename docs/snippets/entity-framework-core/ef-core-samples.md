...
- `SoftDeletePostInTransactionAsync` は更新対象がある場合だけ transaction を開始して `IsDeleted` を更新します。`HasQueryFilter` により通常のクエリから削除済みデータを隠します。
- `UpdatePostTitleWithConcurrencyAsync` は `ConcurrencyStamp` を使って楽観的同時実行制御を行い、モデル制約に合わせて新しい stamp を 40 文字以内に制限します。
...
- これは最小サンプルです。本番ではログ、監査、マイグレーション手順、バックアップ、リトライ方針、例外から API レスポンスへの変換をアプリ要件に合わせて追加してください。
