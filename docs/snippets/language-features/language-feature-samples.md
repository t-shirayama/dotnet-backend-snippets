...
- `src/DotnetBackendSnippets.Core/LanguageFeatures/ByRefAndUnsafeSamples.cs`
...
- unsafe はプロジェクトで許可が必要です。このリポジトリでは、unsafe ブロックを `ByRefAndUnsafeSamples.ReadFirstWithPinnedPointer` の内部だけに閉じ込めています。
